...
const form = useForm({
    title: props.celebration.title,
    description: props.celebration.description || '',
    featured_image: null, // Will only be set if a new image is uploaded
    sort_order: props.celebration.sort_order,
});

const imagePreview = ref(props.celebration.image_url); // Use image_url accessor from model
...
const submit = () => {
    // We must use router.post and spoof the method for file uploads on update
    const data = {
        _method: 'put',
        title: form.title,
    ...
    router.post('/admin/celebrations/' + props.celebration.id, data, {
        preserveScroll: true,
        forceFormData: true,
        onSuccess: () => {
